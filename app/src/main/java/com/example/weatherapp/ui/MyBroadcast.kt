...
class MyBroadcast(val context: Context, val cities: List<Cities>) : LiveData<AppState>() {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            when (intent.getStringExtra(DETAILS_LOAD_RESULT_EXTRA)) {
                ...
                DETAILS_RESPONSE_SUCCESS_EXTRA -> {
                    ...
                    if (forecast != null) {
                        ...
                        location?.let { Location(it) }?.let {
                            value = AppState.Success(Weather(
                                current,
                ...
                DETAILS_DATA_EMPTY_EXTRA -> {
                    ...
                    value = empty?.let { AppState.EmptyData(it) }
                }
                ...
                DETAILS_REQUEST_ERROR_EXTRA -> {
                    ...
                    value = error?.let { AppState.Error(it) }
